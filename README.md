...
Add `ffmpeg_kit` as a dependency in your `pubspec.yaml file`.
...
`shine` library enabled. You have to install a `ffmpeg_kit` package that has at least one of them inside. To
encode an `h264` video, you need to install a package with `x264` inside. To encode `vp8` or `vp9` videos, you need
a `ffmpeg_kit` package with `libvpx` inside.
...
Installing `ffmpeg_kit` enables the `https` package by default. It is possible to install the other packages
...
In order to install the `LTS` variant, append `-LTS` to the version you have for the `ffmpeg_kit` dependency.
...
`ffmpeg_kit` is published in two variants: `Main Release` and `LTS Release`. Both releases share the
